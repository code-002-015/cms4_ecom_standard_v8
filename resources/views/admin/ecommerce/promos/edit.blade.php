                        ...
                        @foreach($brands as $brand)
                            @php
                                ...
                                foreach($products as $p){
                                    $pproducts += \App\Models\PromoProducts::where('promo_id',$promo->id)->where('product_id',$p->id)->count();   
                                }
                            @endphp
                                                    ...
                                                    <table class="table">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            @forelse($products as $product)
                                                                @php
                                                                    $exist = \App\Models\PromoProducts::where('promo_id',$promo->id)->where('product_id',$product->id)->count();
                                                                @endphp
                                                                <tr>
                                                                    <td>{{ $product->name }}</td>
                                                                    <td class="text-right">
                    ...
                    <table class="table table-hover" style="width:100%;">
                        ...
                        @foreach($categories as $category)
                            @if(count($category->published_products) > 0)
                                @php
                                    $cproducts = 0;
                                    $products = \App\Models\Product::where('status','PUBLISHED')->where('category_id',$category->id)->get();
                                    foreach($products as $p){
                                        $cproducts += \App\Models\PromoProducts::where('promo_id',$promo->id)->where('product_id',$p->id)->count();
                                    }
                                @endphp
                                <tr>
